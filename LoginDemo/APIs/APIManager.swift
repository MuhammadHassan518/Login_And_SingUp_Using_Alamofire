...
//                           completionhandler?(status as! Int, data as Any  )
                           let result = try JSONDecoder().decode(UpdateModel_Base.self, from: a!)
                           completionhandler?(status as! Int, result as UpdateModel_Base)
                       }
                       else
                       { print("Invalid data entered") }
                   }
                   catch{ print("erroMsg: \(error)") }
...
//}
//func UploadData(vc:UIViewController,params: [String : Any],img:UIImage, url: String, header: HTTPHeaders, completionhandler:((Int, UpdateModel_Base) -> ())?)  {
//    AF.upload(
//           multipartFormData: { multipartFormData in
//               multipartFormData.append(img.jpegData(
//                   compressionQuality: 0.5)!,
...
//                   let status = 1
//                   let result = try JSONDecoder().decode(UpdateModel_Base.self, from: a!)
//                   completionhandler?(status, result as UpdateModel_Base)
//               }
//               catch { print(error) }
//
//
